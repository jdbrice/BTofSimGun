...
int StBTofSimMaker::InitRun(int runnumber)
{
    ...
    //! Determine if running embedding is active
    fBFChain = dynamic_cast<StBFChain*>(StMaker::GetChain());
    if (fBFChain) {
        if (fBFChain->GetOption("Embedding")) {
            setEmbeddingMode(kTRUE);
        }
    }
...
int StBTofSimMaker::fillEvent()
{
	...
	if (!mIsEmbedding) {  //! send off to StEvent
      ...
      if(!mBTofCollection) {
        ...
        StBTofCollection *btofColl1 = mEvent->btofCollection();
        if (btofColl1) {
            LOG_INFO << "There's something in mEvent->btofCollection()" << endm;
        }
    }
...
//    tof = tof - mSimDb->toffset();  // Apply offset correction.
    ...
    /// X talk signal
    ///
    double tofx = tofHitsFromGeant->tof + ranGauss.shoot()*mSimDb->timeres_tof();    //! 85ps per channel
